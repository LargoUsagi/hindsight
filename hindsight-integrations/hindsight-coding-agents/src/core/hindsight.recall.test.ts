...
describe("HindsightClient.recall", () => {
  it("POSTs to /memories/recall and returns results", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string, init: any) => {
      ...
      return { ok: true, status: 200, json: async () => ({ results: [{ text: "we use zod for validation", scores: { rerank: 0.9 } }] }) } as any;
    }));
    ...
    expect(calls[0].body).toMatchObject({ query: "how do we validate input", max_tokens: 512, budget: "mid" });
  });

  it("returns [] on a non-ok response instead of throwing", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => ({ ok: false, status: 500, text: async () => "err" } as any)));
